        ...
        std::vector<std::vector<uint32_t>> const& edges)
{
    ...
    {
        pasl::pctl::parray<uint32_t> curr_nodes_edges_size(
                frontier.size(),
                [&frontier, &edges](uint32_t node_idx)
                {
                    return edges[frontier[node_idx]].size();
                }
        );
        pasl::pctl::parray<uint32_t> pref_sum = pasl::pctl::scan(
                curr_nodes_edges_size.begin(), curr_nodes_edges_size.end(), static_cast<uint32_t>(0),
        ...
        pasl::pctl::parallel_for(
                static_cast<uint32_t>(0), static_cast<uint32_t>(frontier.size()),
                [&edges, &frontier, &dist, &new_frontier, &pref_sum](uint32_t node_idx)
                {
